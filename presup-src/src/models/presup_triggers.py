...
subroot='E:\\Summer\\presup-src\\src\\presup_ptb'
...
###########################Edited##########################
        ...
        self.extract_negative(corpus,subset,verb_dict,os.path.join(subdir,'negative_data.pkl'))
        print "done extracting negative cases"
        print "extracting positive cases"
        self.extract_positive(corpus,subset,verb_loc, os.path.join(subdir,'positive_data.pkl'))
                            ...
                            if set(target_words).isdisjoint(word_list[j - 3:j + 3]):
                                if verb_dict[one] >= 1:
                                    ...
                                    if j <= 10:
                                        ...
                                        tuple = (0, word_list[j - 10:j]+["@@@@"]+word_list[j:k], pos_list[j - 10:j]+["@@@@"]+pos_list[j:k])
                                    # print tuple
                                    # raw_input("---")
    ...
    def extract_positive(self,corpus,subset,verb_loc,out):
                    ...
                    if one in word_list:
                        ...
                        for i in range(len(indices_of_adverb)):
                            ...
                            if j<=10:
                                ...
                                tuple = (one, word_list[j-10:j]+word_list[j+1:k],pos_list[j-10:j]+pos_list[j+1:k])
                            cPickle.dump(tuple,output)
